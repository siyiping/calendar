...
class DateUtil {
  ...
  static const int oneDayMilliseconds = 24 * 60 * 60 * 1000;
...
//    int lastIndex = lastDayOfMonth.weekday;
  ...
  static String awayFromToday(DateTime day) {
    DateTime now = DateTime(DateTime.now().year, DateTime.now().month, DateTime.now().day);
    if(isCurrentDay(day.year, day.month, day.day)) {
      return "今天";
    } else if(day.isBefore(now)) {
      if ((now.millisecondsSinceEpoch - day.millisecondsSinceEpoch) == oneDayMilliseconds) {
        return "昨天";
      } else {
        return ((now.millisecondsSinceEpoch - day.millisecondsSinceEpoch) ~/ oneDayMilliseconds).toString() + "天前";
      }
    } else if (day.isAfter(now)) {
      if ((day.millisecondsSinceEpoch - now.millisecondsSinceEpoch) == oneDayMilliseconds) {
        return "明天";
      } else {
        return ((day.millisecondsSinceEpoch - now.millisecondsSinceEpoch) ~/ oneDayMilliseconds).toString() + "天后";
      }
    }
    return "";
  }
